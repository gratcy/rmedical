...
$filter				.= "and highlight like '%$user->user%'";

$items				= sql_getTable("select * from board where $filter  order by priority asc, $orderby $ordertype");
/*
...
//		$item->content				= str_replace(array("\t", "  "), array("　　　", "&nbsp; "), nl2br($item->content));
		...
		$notify				= "<a href='#' onclick=\"dialog_popup('dialog_pm'); document.getElementById('dialog_content').src='notify_board_brief.php?id=$item->id'; \"><i class='fa fa-dot-circle-o'></i>";
...
<h1>發送通知</h1>
<iframe id=dialog_content src='notify_board_brief.php?id=$item->id' width=698 height=563 frameborder=0></iframe>
...
move_menu[0] = "<a href='#' onclick='location.href=\"board_brief.php?movefolder=&id=\" + current_item;'> 預設資料夾 </a>";
<?php
...
foreach ($data as $item) {
	...
	echo  "move_menu[$count] = \"<a href='#' onclick='location.href=\\\"board_brief.php?movefolder=$item->id&id=\\\" + current_item;'> $item->name </a>\";";
