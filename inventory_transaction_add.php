...
$site_from			= $_GET['site_from'] * 1;

echo "<h3 class='pull-left'>新增出貨</h3>";


if ($_POST['action'] == 'add') {

	$fields						= sql_secure($_POST, "date, site_from, site_to, type, remark");
	...
	gotoURL("inventory_transaction_edit.php?id=$it_id");
	exit;
...
$inputs->add(
			'date'						, 'text'					, '日期'						, '100%',
			'site_from'					, 'select2'			, '出貨地點'					, '100%',
			'site_to'					, 'select2'			, '收貨地點'					, '100%',
			'type'						, 'select'					, '類型'						, '100%',
...
$inputs->value['site_from']					= $site_from;
$inputs->value['type']						= "一般出貨";


$inputs->options['site_from']				= sql_getArray("select name, id from site order by name");
$inputs->options['site_to']					= $inputs->options['site_from'];
$inputs->options['type']					= array("一般出貨", "轉貸");
...
$inputs->tag['site_from']					= "class='form-control'";
$inputs->tag['site_to']					= "class='form-control'";
