...
function initRTE(imgPath, incPath, css, contentImgPath) {
	//set browser vars
	...
	cssFile				= css;
	contentImagePath	= contentImgPath;
...
/*			if (isIE) {
	...
	//		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="rteCommand(\'' + rte + '\', \'redo\')"></td>');
		...
		document.writeln('<iframe width="284" height="144" id="si' + rte + '" src="' + includesPath + 'select_image.php?path=' + contentImagePath + '" marginwidth="0" marginheight="0" scrolling="no" style="visibility:hidden; position: absolute;" frameborder=0></iframe>');
		document.writeln('<div width="350" height="144" id="slink' + rte + '"  style="visibility:hidden; position: absolute;"><table width=100% cellspacing=7 border=0 bgcolor=#ffffff style=\'border:solid 1px #aaaaaa;\'><tr><td>URL : </td><td><input type=text value=\'\' id=slink_url></td></tr><tr><td>Target:</td><td><input type=checkbox value=_blank id=slink_target> New Window</td></tr><tr><td></td><td><input type=button value=Insert onclick="insertLink(\'\', document.getElementById(\'slink_url\').value, document.getElementById(\'slink_target\').checked); "></td></tr></table></div>');
//		document.writeln('<iframe width="350" height="144" id="slink' + rte + '" src="' + includesPath + 'insert_link.htm" marginwidth="0" marginheight="0" scrolling="no" style="visibility:hidden; position: absolute;"></iframe>');
		...
		if (!readOnly) {
			document.writeln('<textarea class="form-control" name="' + rte + '" id="' + rte + '" style="width: ' + width + 'px; height: ' + height + 'px;">' + html + '</textarea>');
		} else {
			document.writeln('<textarea class="form-control" name="' + rte + '" id="' + rte + '" style="width: ' + width + 'px; height: ' + height + 'px;" readonly>' + html + '</textarea>');
		}
	}
}
...
function rteCommand(rte, command, option, option2) {
...
function dlgColorPalette(rte, command) {
	...
	if ((command == parent.command) && (rte == currentRTE)) {
		...
		for (var i = 0; i < vRTEs.length; i++) {
			...
			showHideElement('si' + vRTEs[i], 'hide');
			showHideElement('slink' + vRTEs[i], 'hide');
		}
		showHideElement(oDialog, 'show');
	}
...
function insertLink(rte, _url, _target) {

	var command = 'insertLink';
...
//	setRange(rte);
	...
	//get dialog position
	var oDialog = document.getElementById('slink' + rte);
	var buttonElement = document.getElementById('hilitecolor_' + rte);
	var iLeftPos = getOffsetLeft(buttonElement);
	var iTopPos = getOffsetTop(buttonElement) + (buttonElement.offsetHeight + 4);
	oDialog.style.left = (iLeftPos) + "px";
	oDialog.style.top = (iTopPos) + "px";
...
//			rteCommand(rte, "Unlink", null);
			...
			//rteCommand(rte, "CreateLink", _url, _target);
			var oRTE;
			if (document.all) {
				oRTE = frames[rte];
			} else {
			...
			if ( oRTE.document.selection ){
				var range = oRTE.document.selection.createRange();
				range.execCommand("CreateLink", 0, _url);
				if (_target)
...
function addImage(rte, imagePath) {
	...
	//get dialog position
	var oDialog = document.getElementById('si' + rte);
	var buttonElement = document.getElementById('hilitecolor_' + rte);
	var iLeftPos = getOffsetLeft(buttonElement);
	var iTopPos = getOffsetTop(buttonElement) + (buttonElement.offsetHeight + 4);
	oDialog.style.left = (iLeftPos) + "px";
