...
if (empty($privilege->edit))	{	gotoURL("user_manage.php"); exit; }
...
if ($_POST['action'] == 'edit') {
	...
	if (empty($error)) {
		sql_query(sql_update("service_user", $fields, "id='$id'"));

		$password		= $_POST['password'];
        if (!empty($password)) {
		sql_query("update service_user set password=md5('$password') where id='$id'");
		}
...
if ($_POST['action'] == 'edit')
	...
	$inputs->value 	= sql_getVar("select name,user,staff_id,store_id from service_user where id='$id'");
}
...
foreach ($items as $item) {
	array2obj($item);
	$value			= sql_getObj("select * from service_user_privilege where `link`='$item->link' and user_id=$id");
