...
$privilege			= sql_getObj("select * from service_user_privilege where user_id='$user->id' and `link`='report_product.php'");
...
if (empty($_GET)) {
	gotoURL("report_product.php?date_select=recent&recent=1&page=staff_group");
	exit;
}
...
if ($show_type == 1) {
	...
	$items				= sql_getTable("
			select
				...
				b.name as name,
				sum(a.item_quantity) as quantity,
	...
	$columns_end		= sql_getObj("
			select
				concat('總產品數：', count(distinct item_id), '') as name_series,
				 '　加總' as name_subitem,
				sum(a.quantity) as quantity,
				sum(a.price) as average_price,
				sum(a.amount_discounted) as average_price_discounted,
				avg(amount_discounted / (price * quantity))*100 as discount_percentage
			from
				invoice_detail a
			where
				$filter and item_id != 0
...
if ($show_type == 2) {
	...
	$columns			= array(
				...
				"quantity"						=> "name:數量			, class:number",
				"average_price"					=> "name:平均折前售價	, class:number",
				"sum_price"						=> "name:總折前售價		, class:number",
				"average_price_discounted"		=> "name:平均折後售價	, class:number",
