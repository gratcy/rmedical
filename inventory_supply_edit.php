...
//	$items_amount				= sql_getArray("select item_id, amount from inventory where site_id='$inventory_transaction->site_from'");
	...
	foreach ($_POST as $name => $value) {
		...
		$items[$item_id]		= $value * -1;		// Because it is site_from , and will be deduced at library code
			...
			foreach ($items as $row) {
				...
				foreach ($row as $item) {
					...
					if ($_SESSION["inventory_newrecord_$it_id"] === true) {
						...
						$amount			= $stock - $items_amount[$item_id];
						...
						echo "<td><input class=number type=text name='item_id_$item_id' value='0' size=1 onfocus='this.select();' tabindex=$item->item_order></td>";
						echo "<td></td>\r\n";
					} else {
