...
echo "<h3 class='pull-left'>編輯出貨</h3>";
...
if ($_POST['action'] == 'edit') {
	...
	inventory_stock_statistic_update($inventory_transaction->site_to);
...
$inputs		= new Inputs();
$inputs->add(
			'date'						, 'text_readonly'			, '日期'						, '100%',
			'site_from'					, 'text_readonly'			, '來源地'						, '100%',
			'site_to'					, 'text_readonly'			, '目的地'						, '100%',
...
$inputs->tag['date']					= "class='form-control' style='width:50%'";
$inputs->tag['site_from']					= "class='form-control' style='width:50%' readonly";
$inputs->tag['site_to']						= "class='form-control' style='width:50%' readonly";
$inputs->tag['type']						= "class='form-control' style='width:50%' readonly";
$inputs->tag['remark']					= "class='form-control' style='width:50%'";
...
//		("select * from inventory a where site_id='$site_from' and exists (select item_id from inventory b where site_id='$site_to' and a.item_id=b.item_id)");
		...
		$sold_start_date	= date("Y-m-d", strtotime($inventory_transaction->date) - 3600 * 24 * 3);
			...
			foreach ($items as $row) {
				...
				foreach ($row as $item) {
					...
					if ($_SESSION["inventory_newrecord_$it_id"] === true) {
						...
						$stock			= sql_getValue("select ifnull(amount, 0) from inventory where site_id='$inventory_transaction->site_to' and item_id='$item_id'");
						...
						$stock			= sql_getObj("select * from inventory_transaction_detail where inventory_transaction_id='$it_id' and site_id='$inventory_transaction->site_to' and item_id='$item_id'");
						$amount			= $items_amount[$item_id];
						echo "<td>$item_name</td>";
						echo "<td><input class=number type=text value='$stock->change_from' size=1 readonly style='color:#777777'></td>";
						echo "<td><input class=number type=text value='$stock->change_to' size=1 readonly style='color:#777777'></td>";
						echo "<td><input class=number type=text name='item_id_$item_id' value='$amount' size=1 onfocus='this.select();' tabindex=$item->item_order></td>";
						echo "<td></td>";
					}
